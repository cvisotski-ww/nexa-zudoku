...
// src/color-vars.ts
import { dashCase, traverse } from "@inkeep/shared";
  ...
  traverse(
    ...
    (_key, value, path) => {
      if (typeof value === "string") {
        const cssVarRef = `--${prefix ? prefix + "-" : ""}colors-` + dashCase(path);
...
var mainColor = new Color(primaryBrandColor);
var createColorDict = (color) => {
  const primary = color instanceof Color ? color : new Color(color);
  ...
  return {
    textBold: new Color("lch", [10, textBoldChroma, primary.lch.h]),
    textSubtle: new Color("lch", [30, textSubtleChroma, primary.lch.h]),
    lighter: new Color("lch", [98, primary.lch.c * 1.2, primary.lch.h * 0.96]),
    light: new Color("lch", [89, primary.lch.c * 1.2, primary.lch.h * 0.96]),
    lightSubtle: new Color("lch", [
      97,
      lightSubtleChroma,
      primary.lch.h * 0.98
    ]),
    medium: primary,
    strong: new Color("lch", [
      primary.lch.l * 0.85,
      primary.lch.c,
      primary.lch.h
    ]),
    stronger: new Color("lch", [
      primary.lch.l * 0.5,
      primary.lch.c,
      primary.lch.h
    ])
...
var createColorScheme = (userScheme, primaryColor) => {
  ...
  Object.keys(merged).forEach((key) => {
    if (merged[key] instanceof Color) {
      return;
    }
    merged[key] = new Color(merged[key]);
  });
