...
export const createOperationSlug = (slugify, operation, tag) => {
...
const cache = new LRUCache({
    ttl: 60 * 10 * 1000,
    ttlAutopurge: true,
    fetchMethod: (_key, _oldValue, { context }) => validate(context),
});
...
const getAllTags = (schema) => {
    ...
    const operationTags = Object.values(schema.paths ?? {})
        ...
        .flatMap((operation) => typeof operation === "object" && "tags" in operation
        ? (operation.tags ?? [])
        : []);
    // Remove duplicates and tags that appear in the schema
    const uniqueOperationTags = [...new Set(operationTags)].filter((tag) => !tags.some((rootTag) => rootTag.name === tag));
    return [...tags, ...uniqueOperationTags.map((tag) => ({ name: tag }))];
};
const getAllOperations = (paths, tag) => {
    const slugify = slugifyWithCounter();
    return Object.entries(paths ?? {}).flatMap(([path, value]) => HttpMethods.flatMap((method) => {
        if (!value?.[method])
            return [];
...
const SchemaTag = builder.objectRef("SchemaTag").implement({
    fields: (t) => ({
        ...
        operations: t.field({
            ...
            resolve: (parent, _args, ctx) => {
                ...
                return getAllOperations(ctx.schema.paths, parent.name).filter((item) => parent.name
        ...
        requestBody: t.field({
            ...
            resolve: (parent) => ({
                ...
                content: Object.entries(parent.requestBody?.content ?? {}).map(([mediaType, content]) => ({
                    ...
                    examples: Object.entries(content.examples ?? {}).map(([name, value]) => ({ name, ...value })),
        ...
        responses: t.field({
            ...
            resolve: (parent) => {
                return Object.entries(parent.responses ?? {}).map(([statusCode, response]) => ({
                    ...
                    content: Object.entries(response.content ?? {}).map(([mediaType, mediaTypeObject]) => ({
                        mediaType,
                        schema: mediaTypeObject.schema,
                        examples: Object.entries(mediaTypeObject.examples ?? {}).map(([name, value]) => ({ name, ...value })),
                    })),
...
const Schema = builder.objectRef("Schema").implement({
    fields: (t) => ({
        ...
        operations: t.field({
            ...
            resolve: (parent, args) => getAllOperations(parent.paths).filter((item) => (!args.operationId || item.operationId === args.operationId) &&
...
const loadOpenAPISchema = async (input) => {
...
export const createGraphQLServer = (options) => createYoga({ schema, ...options });
