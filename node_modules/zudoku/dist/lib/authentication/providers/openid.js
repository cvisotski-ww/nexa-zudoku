    ...
    getRoutes() {
        return [
            ...super.getRoutes(),
            {
                ...
                element: _jsx(CallbackHandler, { handleCallback: this.handleCallback }),
            },
        ];
    }
}
export class OpenIDAuthenticationProvider {
    ...
    audience;
    constructor({ issuer, audience, clientId, redirectToAfterSignUp, redirectToAfterSignIn, redirectToAfterSignOut, }) {
        this.client = {
            client_id: clientId,
    ...
    async getAuthServer() {
        if (!this.authorizationServer) {
            ...
            this.authorizationServer = await oauth.processDiscoveryResponse(issuerUrl, response);
        }
    ...
    async getAccessToken() {
        ...
        const state = JSON.parse(tokenState);
        if (state.expiresOn < new Date()) {
            if (!state.refreshToken) {
                await this.signIn();
                return "";
            }
            const request = await oauth.refreshTokenGrantRequest(as, this.client, state.refreshToken);
            const response = await oauth.processRefreshTokenResponse(as, this.client, request);
    ...
    signOut = async () => {
        ...
        const as = await this.getAuthServer();
        const redirectUrl = new URL(window.location.origin + this.redirectToAfterSignOut);
    ...
    handleCallback = async () => {
        ...
        const params = oauth.validateAuthResponse(authServer, this.client, url.searchParams, state ?? undefined);
        ...
        // }
        const oauthResult = await oauth.processAuthorizationCodeOpenIDResponse(authServer, this.client, response);
        this.setTokensFromResponse(oauthResult);
        const accessToken = await this.getAccessToken();
        const userInfoResponse = await oauth.userInfoRequest(authServer, this.client, accessToken);
        ...
        sessionStorage.setItem("profile-state", JSON.stringify(useAuthState.getState().profile));
