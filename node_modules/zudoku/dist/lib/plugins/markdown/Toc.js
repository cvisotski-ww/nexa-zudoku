...
const TocItem = ({ item, children, className, isActive, }) => {
    return (_jsxs("li", { className: cn("truncate", isActive
            ? "text-primary"
            : "text-foreground/65 dark:text-foreground/75", className), title: item.value, children: [_jsx(AnchorLink, { to: `#${item.id}`, [DATA_ANCHOR_ATTR]: item.id, className: cn(isActive
                    ? "text-primary"
                    : "text-foreground/65 dark:text-foreground/75 hover:text-foreground"), children: item.value }), children] }));
};
