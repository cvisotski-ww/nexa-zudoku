...
export const SyntaxHighlight = ({ copyable = true, language = "plain", ...props }) => {
    ...
    return (_jsx(Highlight, { theme: isDark ? themes.vsDark : themes.github, language: language, ...props, children: ({ className, style, tokens, getLineProps, getTokenProps }) => (_jsxs("div", { className: "relative group", children: [_jsx("pre", { className: cn("relative scrollbar overflow-x-auto", className, props.className, props.noBackground && "!bg-transparent", props.wrapLines && "whitespace-pre-wrap break-words"), style: style, children: tokens.map((line, i) => (
                    ...
                    _jsx("div", { ...getLineProps({ line }), children: line.map((token, key) => (
                        // eslint-disable-next-line react/no-array-index-key
                        _jsx("span", { ...getTokenProps({ token }) }, key))) }, i))) }), props.showLanguageIndicator && (_jsx("span", { className: "absolute top-1.5 right-3 text-[11px] font-mono text-muted-foreground transition group-hover:opacity-0", children: language })), copyable && (_jsx("button", { type: "button", "aria-label": "Copy code", title: "Copy code", className: "absolute top-2 right-2 p-2 opacity-0 group-hover:opacity-100 group-hover:bg-zinc-100 group-hover:dark:bg-zinc-700 hover:outline hover:outline-border/75 dark:hover:outline-border rounded-md text-sm text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-400 transition", disabled: isCopied, onClick: () => {
                        setIsCopied(true);
                        void navigator.clipboard.writeText(tokens
                            .map((line) => line.map(({ content }) => content).join(""))
                            .join("\n"));
                        setTimeout(() => setIsCopied(false), 2000);
                    }, children: isCopied ? (_jsx(CheckIcon, { className: "text-emerald-600", size: 16 })) : (_jsx(CopyIcon, { size: 16 })) }))] })) }));
};
