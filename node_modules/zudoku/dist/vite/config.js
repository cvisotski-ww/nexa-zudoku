...
const fileExists = (path) => stat(path)
    ...
    .catch(() => false);
let configPath;
export async function getConfigFilePath(rootDir) {
...
let config;
export async function loadZudokuConfig(rootDir, forceReload) {
    ...
    try {
        ...
        const loadedConfig = await tsImport(configFilePath, {
            ...
            onImport: (file) => {
                const path = fileURLToPath(file.startsWith("file://") ? file : pathToFileURL(file).href);
...
export function getPluginOptions({ dir, mode, }) {
...
export async function getViteConfig(dir, configEnv, onConfigChange) {
    ...
    const viteConfig = {
        ...
        build: {
            ...
            sourcemap: true,
            outDir: path.resolve(path.join(dir, "dist", configEnv.isSsrBuild ? "server" : "")),
            emptyOutDir: true,
            rollupOptions: {
                input: configEnv.command === "build"
                ...
                plugins: [
                    tailwindcss({
                        ...
                        content: [
                            ...
                            config.__meta.path,
                            ...config.__meta.dependencies.map((dep) => `${path.dirname(config.__meta.path)}/${dep}`),
    ...
    if (userConfig) {
        const merged = mergeConfig(userConfig.config, viteConfig, true);
        return merged;
    }
    return viteConfig;
}
