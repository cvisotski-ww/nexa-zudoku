...
const ApiKeysSchema = z.union([
    ...
    z.object({
        enabled: z.boolean(),
        getKeys: z.custom((val) => typeof val === "function"),
        rollKey: z
...
const ConfigSchema = z
    ...
    page: z
        ...
        .partial(),
    topNavigation: z.array(z.object({
        label: z.string(),
        id: z.string(),
    ...
    authentication: z.union([
        z.object({
            ...
            clerkPubKey: z.custom((val) => typeof val === "string" ? /^pk_(test|live)_\w+$/.test(val) : false),
    ...
    if (!config.sidebar || !config.topNavigation)
        return;
    const topNavIds = config.topNavigation.map((item) => item.id);
    const nonExistentKeys = Object.keys(config.sidebar).filter((key) => !topNavIds.includes(key));
