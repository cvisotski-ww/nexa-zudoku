...
export const convertZudokuConfigToOptions = (config) => {
    const fallbackLogoLight = config.page?.logoUrl ??
        "https://cdn.zudoku.dev/logos/zudoku-logo-full-light.svg";
    const fallbackLogoDark = config.page?.logoUrl ??
        "https://cdn.zudoku.dev/logos/zudoku-logo-full-dark.svg";
    const isUsingFallback = !config.page?.logoUrl &&
...
export const getRoutesByConfig = (config) => {
    ...
    const topNavRedirects = options.topNavigation?.flatMap((topNavItem) => {
        if (!options.sidebars?.[topNavItem.id])
            return [];
        const first = topNavItem.default ??
            traverseSidebar(options.sidebars[topNavItem.id], (item) => {
                if (item.type === "doc")
                    return joinPath(topNavItem.id, item.id);
            });
        if (!first)
            return [];
        return {
    ...
    return [
        {
            element: (_jsx(DevPortal, { ...options, children: _jsx(Layout, {}) })),
            children: [
                {
